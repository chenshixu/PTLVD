...
def generate_complete_json(data_nodes, complete_pdg_path, func_name):
    ...
    for node in data_nodes:   # 生成pdg图的节点
        node_id = '"' + data_nodes[node].id.split("id=")[-1][:-1] + '"' # id编号
        node_type = data_nodes[node].node_type      # id label类型 如 MethodReturn
        node_code = data_nodes[node].properties.code()  # code
        node_label = "("+node_type+','+node_code+")"    # 如(MethodReturn,代码)
        dot_node = pydot.Node(node_id, label = node_label)  # 如 "1000168" [label="(MethodReturn,static int)"]
        graph.add_node(dot_node)
    for node in data_nodes:     # 生成pdg图的边
        node_edges = data_nodes[node].edges
        for node_edge in node_edges:
            node_edge_label = node_edges[node_edge].type  # 边的类型 如Ast
            node_in_id = '"' + node_edges[node_edge].node_in.split("id=")[-1][:-1] + '"'        # in节点id编号
            node_out_id = '"' + node_edges[node_edge].node_out.split("id=")[-1][:-1] + '"'      # out节点id编号
            ...
            dot_edge = pydot.Edge(node_in_id, node_out_id, label = node_edge_label)     # 如
...
def generate_sub_json(all_data_nodes, _point_slice_list, sub_graph_path, func_name, points_name, label_path):
    ...
    if func_name.startswith("1_"):  # 得到漏洞行信息
    ...
    for subgraph in _point_slice_list:
        ...
        line_num_list = list(set(line_num_list))  # set类型去重复数据
        if len(line_num_list) == 1:
            continue # 排除切片内只有一行节点的切片

        subgraph_tmp = data_nodes
        for node in subgraph_tmp[:]:    # 遍历
        ...
        graph = pydot.Dot(func_name, graph_type = 'digraph')
        for node in data_nodes:  # 与生成pdg的流程类似
            node_id = '"' + node.id.split("id=")[-1][:-1] + '"'
            node_record.append(node_id)     # 存储id编号
        ...
        for node in data_nodes:
            ...
            for node_edge in node_edges:
                ...
                if node_edge_label == 'Ast':
                    ...
                    ddg_var = node_edge.split("@")[-1].split("#")[0]
                    node_edge_label = 'DDG: ' + ddg_var
                _edge_info = [node_in_id, node_out_id, node_edge_label] # 存储ddg边
                if _edge_info not in edge_record:
                    edge_record.append([node_in_id, node_out_id, node_edge_label])  # 存储所有ddg、ast、cfg边

        left_edge_node_list = []
        for edge_info in edge_record:   # 遍历边
            for edge_node_id in [edge_info[0],edge_info[1]]:
                edge_node_id = edge_node_id[1:-1]       # 得到节点id
