...
def get_local_node(node_dict):
    pointer_node_list = []
    identifier_list = []
    identifier_node_type = ['', 'MethodParameterIn', 'FieldIdentifier']
...
def get_pointers_node(node_dict):
    ...
    for node in node_dict:
        node_type = node_dict[node].node_type
        if node_type in identifier_node_type:  # 查询是否为几个类型
            identifier_list.append(node_dict[node])
    for node in identifier_list:
        node_code = node.properties.code()
        indx_1 = node_code.find("*")
        node_has_full_type = node.properties.has_type()  # properties的type_fullName存在
        if node_has_full_type != False:
            node_full_type = node.properties.get_type()     # 得到指针类型 如 struct inode*
            indx_2 = node_full_type.find("*")   # 再次查看是否存在*
        else:
            indx_2 = -1
       
        if indx_1 != -1 or indx_2 != -1:  # 确定为指针,将节点添加到列表
            pointer_node_list.append(node)
    pointer_node_list = list(set(pointer_node_list))
    return pointer_node_list
