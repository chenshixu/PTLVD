...
def joern_parse(file, outdir):
    ...
    os.system('/home/chen/source/joern-cli/joern-parse/joern-parse $file --language c --out $out')
...
def joern_export(bin, outdir, repr):
    ...
    if repr == 'pdg':
        os.system('/home/chen/source/joern-cli/joern-export $bin'+ " --repr " + "pdg" + ' --out $out') # cpg 改成 pdg
        try:
        ...
        script_path = f"/home/chen/graph-for-funcs.sc"
...
def main():
    joern_path = '/home/chen/source/joern-cli/'
    os.chdir(joern_path)
    args = parse_options()
    type = args.type
    repr = args.repr
...
if __name__ == '__main__':
    # main()
    bin_path =  '/mnt/d/source/mvul_gadgets/v100/bins/'
    out_dir = '/mnt/d/source/mvul_gadgets/v100/jsons/'
    pdg_path = '/mnt/d/source/mvul_gadgets/v100/pdgs/'
    pdgs = glob.glob(pdg_path + '*.dot')
    bins = []
    for pdg in pdgs:
        bins.append(bin_path+pdg.split('/')[-1].split('.')[0]+'.bin')
        if not os.path.exists(bin_path+pdg.split('/')[-1].split('.')[0]+'.bin'):
            print('eror')
    with Manager():
        pool = Pool(20)
        process_func = functools.partial(joern_export, outdir=out_dir,repr='lineinfo_json')
        dirs = [dir for dir in tqdm(pool.imap_unordered(process_func, bins), desc=f"jsons: ", total=len(bins), )]
        pool.close()
        pool.join()